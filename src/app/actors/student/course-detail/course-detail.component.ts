...
export class CourseDetailComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    try{
      let result = await this.courseService.getCourse(courseId, this.auth.loggedInUser().position);
  ...
  getTimeDifference(createdDate, dueDate){
    let result = new Date(dueDate).getTime() - new Date(createdDate).getTime();
    return result;
  }
  
  isExpired(dueDate: string){
    let dateNow = new Date().getTime();
    let dueDateN = new Date(dueDate).getTime();

    let differnceInHour = (dueDateN - dateNow)/(1000 * 60 * 60);
