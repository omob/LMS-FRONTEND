...
export class CoursesComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    try {
     ...
     console.log(this.sessionsList)
      let __session = this.getLatestSession(this.sessionsArray); //gets the current session i.e latest session: Session[]
      ...
      let semesterArray = this.getLatestSemester(this.getLatestSession(this.sessionsArray).semesters); //gets current semester
  ...
  private getCourses(session?: string, semester?: string){
    ...
    let _session = this.getSession(this.sessionsArray); 
    this.semestersList = this.getSemesters(_session.semesters);
    
    let _semester = this.getSemester(_session.semesters);
...
// private functions for computing the session and semester 
  ...
  private getLatestSession(sessions: Session[]){
    ...
    let _session = sessions[sessions.length - 1];
    return {
