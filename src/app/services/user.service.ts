...
export class UserService extends DataService {
  ...
  async uploadDoc(role: string, formData: FormData): Promise<ServerResponse>{
    return this.http.post(this.getUrl(role) + '/profile/uploads', formData, { headers: { 'Authorization': this.auth.getToken() }}).toPromise();
  }

  async removeDoc(role: string, doc): Promise<ServerResponse>{
    return this.http.delete(this.getUrl(role)+'/profile/uploads', {  headers: { 'Authorization': this.auth.getToken() }, params: {document: JSON.stringify(doc) }} ).toPromise();
  }
